...
def search(parsed_data, y, x):
    ...
    if x + 3 <= len(parsed_data[y]):
        if (
            parsed_data[y][x + 1] == "M"
            and parsed_data[y][x + 2] == "A"
            and parsed_data[y][x + 3] == "S"
        ):
            true_count += 1
        ...
        if y + 3 <= len(parsed_data) - 1:
            if (
                parsed_data[y + 1][x + 1] == "M"
                and parsed_data[y + 2][x + 2] == "A"
                and parsed_data[y + 3][x + 3] == "S"
            ):
                true_count += 1
            # Forward Up
        if y - 3 > -1:
    ...
    if y - 3 > -1:
        if (
            parsed_data[y - 1][x] == "M"
            and parsed_data[y - 2][x] == "A"
            and parsed_data[y - 3][x] == "S"
        ):
            true_count += 1
...
def xmas(parsed_data, y, x):
    ...
    if y - 1 > -1 and y + 1 < len(parsed_data):
        ...
        if x - 1 > -1 and x + 1 < len(parsed_data[y]) - 1:
            ...
            if (
                parsed_data[y - 1][x - 1] == "M"
                and parsed_data[y - 1][x + 1] == "M"
                and parsed_data[y + 1][x - 1] == "S"
                and parsed_data[y + 1][x + 1] == "S"
            ):
                true_count += 1
            elif (
                parsed_data[y - 1][x - 1] == "S"
                and parsed_data[y - 1][x + 1] == "S"
                and parsed_data[y + 1][x - 1] == "M"
                and parsed_data[y + 1][x + 1] == "M"
            ):
