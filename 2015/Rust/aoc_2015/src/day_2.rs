...
fn part_1() {
    ...
    for i in 0..parts.len() - 1 {
        ...
        let x: i32 = prezzies[0].parse().expect("Couldn't convert to i32.");
        let y: i32 = prezzies[1].parse().expect("Couldn't convert to i32.");
        let z: i32 = prezzies[2].parse().expect("Couldn't convert to i32.");

        //2(lw + wh + hl)
        let prezzie_dimen: i32 = (2 * (x * y)) + (2 * (y * z)) + (2 * (z * x));

        //Slack
        let slack: i32 = if x * y <= y * z && x * y <= z * x {
            x * y
...
fn part_2() {
    ...
    println!("Total prezzie dimensions in Part 2: {}", total_dims);
}
